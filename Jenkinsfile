pipeline {
    ...
    stages {
        stage('SCM Checkout') {
            steps {
                ...
                // git 'https://github.com/Mayur-tiwari/BankingAppp.git'
		        checkout scm
            }
		}
        stage('Maven Build') {
        ...
        stage("Docker build") {
            steps {
                sh 'docker version'
                sh "docker build -t mayurtiwari/bankapp-eta-app:${BUILD_NUMBER} ."
                sh 'docker image list'
                sh "docker tag mayurtiwari/bankapp-eta-app:${BUILD_NUMBER} mayurtiwari/bankapp-eta-app:latest"
            }
        }
    ...
    steps {
		...
		sh "docker push mayurtiwari/bankapp-eta-app:latest"
            }
        }
    }
}
